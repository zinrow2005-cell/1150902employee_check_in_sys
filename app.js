(function(){
  'use strict';
  // W444 FIX380 | leave UX guard + roster-change feedback + work-data guidance.
  ...
  function renderWorkPlan(){
    ...
    if(!rows.length){const known=workPlanRows().length,available=workPlanAvailableMonths();list.innerHTML=`<div class="work-plan-empty"><b>${esc(month)} 尚未收到可顯示的重大工作事項</b><span>${state.workPlanDepartment==='all'?'目前全場工作項目為空。':'目前此部門沒有工作項目。'}</span><small>員工端目前共收到 ${known} 項；可用月份：${available.length?esc(available.join('、')):'尚無'}。請主管到主系統「生產批次事件月曆」按「重新產生月曆」，再到「出勤管理中心」按「同步打卡＋員工自助中心」。</small></div>`;return;}
  ...
  function renderTasks(){
    ...
    if(!rows.length){const total=(state.portal?.workTasks||[]).length;el.innerHTML=`<div class="work-plan-empty"><b>${total?'目前篩選條件下沒有工作任務':'目前尚未收到指派給本人的工作任務'}</b><span>${total?'可切換上方「全部任務」查看其他狀態。':'「工作項目表」是全場重大工作；「我的工作任務」必須由主系統完成具名人力配置與薪資排班審核後才會產生。'}</span>${total?'': '<small>主管操作：主系統「生產批次事件月曆」→「下一步：全場工作與人力排程」→「人力配置與排班橋接」指定主責／支援員工 → 完成審核 → 再到「出勤管理中心」按「同步打卡＋員工自助中心」。</small>'}</div>`;return;}
